...
# دالة Quadratic Kernel
def quadratic_kernel(x1, x2, gamma=1.0, c=1.0):
    dot = sum(float(a) * float(b) for a, b in zip(x1, x2))
    return (gamma * dot + c) ** 2
...
class KernelLogisticRegression:
    def __init__(self, learning_rate=0.01, epochs=1000, gamma=1.0, c=1.0):
    ...
    def predict(self, X_train, X_test):
        ...
        for x in X_test:
            linear_sum = sum(
                self.alpha[j] * quadratic_kernel(X_train[j], x, gamma=self.gamma, c=self.c)
...
if __name__ == "__main__":
    ...
    model = KernelLogisticRegression(learning_rate=0.01, epochs=300, gamma=1.0, c=1.0)
